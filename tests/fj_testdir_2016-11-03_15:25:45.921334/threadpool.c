...
static void * thread_runner(void *t) {
    ...
    while(!thread->pool->shutDown){
        ...
        if(!list_empty(&thread->queue)) {
            ...
            f->futureState = WORKING;
            f->thiefNumber = thread->threadNum;
            pthread_mutex_unlock(&f->futureStateLock);
...
void * future_get(struct future *f) {
	...
	if(f->futureState == DONE) {
        ...
        if(internalExternal == 0) {//main thread is helping

            if(f->thiefNumber == 0) {//main thread is the one that stole
                if(!list_empty(&f->pool->globalQueue)) {
                    pthread_mutex_unlock(&f->futureStateLock);// dont need this locked anymore
                    //pop from global queue
                    pthread_mutex_lock(&list_entry(list_rbegin(&f->pool->globalQueue),struct future, e)->futureStateLock);
                    struct list_elem *elem = list_pop_back(&f->pool->globalQueue);
                    struct future *fut = list_entry(elem, struct future, e);

                    fut->futureState = WORKING;
                    fut->thiefNumber = internalExternal;
                    //pthread_cond_signal(&future_cond);

                    pthread_mutex_unlock(&fut->futureStateLock);
                    pthread_mutex_unlock(&f->pool->globalQueueLock);
                    run_future(fut);
                    return fut;
                }
            ...
            else{//the thief is someone else
                pthread_mutex_lock(&f->pool->threads[f->thiefNumber-1].queueLock);
                pthread_mutex_unlock(&f->pool->globalQueueLock);
                if(!list_empty(&f->pool->threads[f->thiefNumber-1].queue)) {
                    pthread_mutex_unlock(&f->futureStateLock);// dont need this locked anymore
                    int temp = f->thiefNumber;
                    pthread_mutex_lock(&list_entry(list_rbegin(&f->pool->threads[f->thiefNumber-1].queue),struct future, e)->futureStateLock);
                    struct list_elem *elem = list_pop_back(&f->pool->threads[f->thiefNumber-1].queue);
                    struct future *fut = list_entry(elem, struct future, e);
            ...
            if(!list_empty(&f->pool->globalQueue)) {
                ...
                f->futureState = WORKING;
                f->thiefNumber = internalExternal;
                //pthread_cond_signal(&future_cond);
            ...
            if(!list_empty(&f->pool->threads[f->threadRunningF-1].queue)) {
                pthread_mutex_unlock(&f->futureStateLock);// dont need this locked anymore
                pthread_mutex_lock(&list_entry(list_rbegin(&f->pool->threads[f->threadRunningF-1].queue),struct future, e)->futureStateLock);
                struct list_elem *elem = list_pop_back(&f->pool->threads[f->threadRunningF-1].queue);
                struct future *f = list_entry(elem, struct future, e);
