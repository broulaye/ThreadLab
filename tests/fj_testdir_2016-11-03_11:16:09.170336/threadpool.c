...
void * future_get(struct future *f) {
	...
	if(f->futureState == DONE) {
        ...
        */
        ...
        if(f->threadRunningF == 0) {
            printf("Unlocked global queue lock while internal external = %d\n", internalExternal);
            pthread_mutex_unlock(&f->pool->globalQueueLock);
        }
        else {
            pthread_mutex_unlock(&f->pool->threads[f->threadRunningF-1].queueLock);
        ...
        if(internalExternal == 0) {
            ...
            if(list_size(&f->pool->globalQueue)>0) {//got more work on your queue
                pthread_mutex_unlock(&f->futureStateLock);
                pthread_mutex_lock(&list_entry(list_begin(&f->pool->globalQueue),struct future, e)->futureStateLock);
                struct list_elem *elem = list_pop_front(&f->pool->globalQueue);
                struct future *fut = list_entry(elem, struct future, e);

                fut->futureState = WORKING;
                fut->threadRunningF = internalExternal;
                //pthread_cond_signal(&future_cond);
            ...
            if(list_size(&f->pool->threads[internalExternal-1].queue)>0) {
                pthread_mutex_unlock(&f->futureStateLock);
                pthread_mutex_lock(&list_entry(list_begin(&f->pool->threads[internalExternal-1].queue),struct future, e)->futureStateLock);
                struct list_elem *elem = list_pop_back(&f->pool->threads[internalExternal-1].queue);
                struct future *fut = list_entry(elem, struct future, e);
